...
import com.htc.htcwalletsdk.Utils.VolleyUtils;
...
public class HtcWalletSdkManager implements ISdkExportKeyAgentApis {
    ...
    /**
     * Get a public Key from SDK for send purpose,and if call this API, the index string of path will auto increase always.
    ...
    /**
     * Get public Key from SDK for receive purpose,and if call this API, the index string of path will auto increase always.
    ...
    public PublicKeyHolder getReceivePublicKey(long unique_id, int coinType, int index) {
        ...
        SdkProtector.throwIfOnMainThread(bAvoidUiThread);
        SdkProtector.throwIfSdkNotInitial( SingletonHolder.bInit);
        PublicKeyHolder publicKeyHolder = mKeyAgent.getPublicKey(unique_id, coinType, 1, index); // pur0:send,  1:receive
        ZKMALog.i(TAG, "getReceivePublicKey idx ---");
        return publicKeyHolder;
    }

    /**
     * Get public Key from SDK for send purpose,and if call this API, the index string of path will auto increase always.
     *  In case of BTC/LTC, the path will be m/44'/0'/0'/0/index
     *  In case of ETH, the path will be m/44'/60'/0'/0/0
     *  In case of XLM, the path will be m/44'/148'/0'/0'/0'
     * Note: To prevent TRUST-ZONE blocked main thread, the caller must call this API at background thread.
    ...
    /**
     * download data from server by {@link JsonObjectRequest }
     * Note: To prevent TRUST-ZONE blocked main thread, the caller must call this API at background thread.
     *
     * @param context  A {@link Context} to use for creating the cache dir.
     * @param jsonObjectRequest A {@link JsonObjectRequest} request to download service
     *
     * @exception UnsupportedOperationException if API not support by SDK.
     * @exception IllegalStateException if API called in UI thread.
     * @exception RuntimeException if SDK not init yet.
    ...
    @Deprecated
    @WorkerThread
    public void downloadServerData(Context context, JsonObjectRequest jsonObjectRequest ) {
        ZKMALog.i(TAG, "downloadServerData +++");
        SdkProtector.throwIfApiNotSupport();
        SdkProtector.throwIfOnMainThread(bAvoidUiThread);
        SdkProtector.throwIfSdkNotInitial( SingletonHolder.bInit);
        VolleyUtils.downloadData(context, jsonObjectRequest);
        ZKMALog.i(TAG, "downloadServerData ---");
    }
